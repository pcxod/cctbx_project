...
class ImageImporterBase():
  ''' Base class for image importer, which will:
        ...
        3. output a experiment list or file for processing
  '''
  ...
  def load_image_file(self, filepath):
    ...
    :return: experiments, error message (if any)
    '''
    # Create experiment list from file
    try:
      experiments = ExperimentListFactory.from_filenames(filenames=[filepath])
  ...
  def write_to_file(self, experiments, filepath):
    filename = os.path.basename(filepath)
    dump = ExperimentListDumper(experiments)
    dump.as_json(filename=filename)
  ...
  def import_image(self, input_entry):
    ...
    self.experiments, error = self.calculate_parameters(experiments=self.experiments)
    if error:
      self.update_log(data=self.experiments, msg=error)
